    ...
    <form id="addForm" class="addForm" method="POST" action="{{ route('certificates.store') }}">
        ...
        <div class="form-group">
            <div class="row">
                ...
                <div class="col-md-6">
                    <label for="certificate_type_id" class="form-control-label">{{ trans('admin.diploma_name') }}</label>
                    <select class="form-control" name="certificate_type_id">
                        <option value="" selected disabled>@lang('admin.select')</option>
                        @foreach ($certificate_types as $certificate_type)
                            <option value="{{ $certificate_type->id }}">
                                {{ lang() == 'ar' ? $certificate_type->certificate_type_ar : $certificate_type->certificate_type_en }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="row">
                <div class="col-md-6 mt-3">
                    <label for="validation_year" class="form-control-label">{{ trans('admin.validation_year') }}</label>
                    <input type="number" name="validation_year" class="form-control">
                </div>

                <div class="col-md-6 mt-3">
                    <label for="year" class="form-control-label">{{ trans('admin.diploma_year') }}</label>
