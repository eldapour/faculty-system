                ...
                <div class="col-md-6">
                    ...
                    <select name="department_branch_id" class="form-control" required>
                        @foreach ($data['department_branchs'] as $key => $department_branch)
                            <option value="{{ $department_branch->id }}" style="text-align: center" {{ $element->department_branch_id == $department_branch->id ? 'selected' : '' }}>{{ $department_branch->branch_name[lang()] }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
