                ...
                <div class="col-md-6">
                    ...
                    <select name="period" class="form-control">
                        <option value="ربيعيه" style="text-align: center"
                            {{ $processExam->period == 'ربيعيه' ? 'selected' : '' }}>{{ trans('admin.autumnal') }}
                        </option>
                        <option value="خريفيه" style="text-align: center"
                            {{ $processExam->period == 'خريفيه' ? 'selected' : '' }}>{{ trans('admin.fall') }}</option>
                    </select>
                </div>
                <div class="col-md-6">
                    <label for="year">{{ trans('admin.year') }}</label>
                    <input type="number" value="{{ $processExam->year }}" class="form-control" name="year">
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    ...
                    <select name="request_status" class="form-control">
                        <option value="new" style="text-align: center"
                            {{ $processExam->request_status == 'new' ? 'selected' : '' }}>{{ trans('admin.new') }}
                        </option>
                        <option value="accept" style="text-align: center"
                            {{ $processExam->request_status == 'accept' ? 'selected' : '' }}>
                            {{ trans('admin.accept') }}
                        </option>
                        <option value="refused" style="text-align: center"
                            {{ $processExam->request_status == 'refused' ? 'selected' : '' }}>
                            {{ trans('admin.refused') }}
                        </option>
                        <option value="under_processing" style="text-align: center"
                            {{ $processExam->request_status == 'under_processing' ? 'selected' : '' }}>
                            {{ trans('admin.under_processing') }}</option>
                    </select>
                </div>
                <div class="col-md-6">
                    <label for="year">{{ trans('admin.request_date') }}</label>
                    <input type="date" value="{{ $processExam->request_date }}" class="form-control"
                        name="request_date">
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
                    ...
                    <input type="date" value="{{ $processExam->processing_request_date }}"
                        name="processing_request_date" class="form-control" />
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
